...
class options {
	...
	protected static function sanitize(&$options) {
		...
		if (
			!static::$pro ||
			!in_array($options['send']['method'], static::SEND_METHODS, true)
		) {
		...
		// CRON sending.
		if (!static::$pro || !$options['send_queue']['enabled']) {
		...
		if (
			!static::$pro ||
			!in_array($options['send_data']['method'], static::DATA_METHODS, true)
		) {
	...
	public static function get_editor_theme() {
		if (is_null(static::$editor_theme)) {
	...
	public static function get_default_roles($refresh=false) {
		if ($refresh || is_null(static::$roles)) {
			static::$roles = array();
	...
	public static function has($key, $refresh=false) {
		if (is_null(static::$has) || $refresh) {
			...
			if ('nothing' !== get_option('wh_db_version', 'nothing')) {
				...
				static::$has['errors'] = !is_null($wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_message_errors` LIMIT 1"));
				static::$has['messages'] = !is_null($wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_messages` LIMIT 1"));
				static::$has['queue'] = !is_null($wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_message_queue` LIMIT 1"));

				// Some of these only exist if there are messages...
				if (static::$has['messages']) {
					static::$has['content'] = !is_null($wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_messages` WHERE LENGTH(`message`) LIMIT 1"));
					static::$has['links'] = !is_null($wpdb->get_var("SELECT `mask` FROM `{$wpdb->prefix}wh_message_links` LIMIT 1"));
				}
			}
		}
