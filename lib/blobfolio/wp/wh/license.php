...
use \blobfolio\wp\wh\vendor\domain;

class license {

	const ITEM = 'Well-Handled';
	const ITEM_ID = 'well-handled';
	...
	protected function decode_local() {
		...
		try {
			$key = file_get_contents(WH_BASE . 'public.pem');
	...
	protected function needs_remote_sync() {
		...
		if (is_null($this->sync)) {
			$tmp = get_option('wh_remote_sync', array('what'=>'', 'when'=>0));
			$this->sync = (
				!is_array($tmp) ||
				!isset($tmp['what']) ||
				!isset($tmp['when']) ||
	...
	protected function decode_remote() {
		...
		// super-critical.
		update_option('wh_remote_sync', array('what'=>$this->hash, 'when'=>strtotime('+2 weeks')));
