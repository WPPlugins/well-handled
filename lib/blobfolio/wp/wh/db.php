  ...
  KEY date_scheduled (date_scheduled)
	...
	public static function upgrade($version=null) {
		...
		if (is_string($version) && $version && version_compare($version, '0.4.2') < 0) {
			...
			if (!is_null($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}wh_messages'"))) {
				$wpdb->query("ALTER TABLE `{$wpdb->prefix}wh_messages`
			...
			if (!is_null($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}wh_message_links'"))) {
				$wpdb->query("ALTER TABLE `{$wpdb->prefix}wh_message_links`
