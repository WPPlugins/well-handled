...
class message {
	...
	public static function get($message_id=null, $refresh=false) {
		...
		if ($refresh || !isset(self::$_instances[$class][$message_id])) {
			self::$_instances[$class][$message_id] = new static($message_id);
			if (!self::$_instances[$class][$message_id]->is_message()) {
				unset(self::$_instances[$class][$message_id]);
				return new static();
			}
		}

		return self::$_instances[$class][$message_id];
	}
	...
	public function __construct($message_id=null) {
		...
		// And get the content.
		$dbResult = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}wh_messages` WHERE `id`=$message_id", ARRAY_A);
	...
	public static function get_message_id_from_mask($mask) {
		...
		if (strlen($mask) === 20) {
			$message_id = (int) $wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_messages` WHERE `mask`='$mask'");
			if ($message_id) {
				return $message_id;
			}
		}
	...
	public function __call($method, $args) {
		...
		throw new \Exception(sprintf(__('The required method "%s" does not exist for %s', 'well-handled'), $method, get_called_class()));
	}
	...
	public function get_links($refresh=false) {
		...
		if ($refresh || is_null($this->links)) {
			$this->links = array();
			global $wpdb;
			$dbResult = $wpdb->get_results("SELECT `mask` FROM `{$wpdb->prefix}wh_message_links` WHERE `message_id`={$this->message['id']} ORDER BY `url` ASC", ARRAY_A);
			if (is_array($dbResult) && count($dbResult)) {
	...
	public static function message_mask() {
		...
		while (intval($wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_messages` WHERE `mask`='$mask'"))) {
			$mask = common\data::random_string(20);
		}
	...
	public static function send_now($to, $subject, $message, $headers, $attachments, $testmode=false, $build=null) {
		...
		if (in_array($send['method'], array('smtp','wp_mail'), true)) {
			add_filter('wp_mail_content_type', array(get_called_class(), 'mail_content_type'));
			if ('smtp' === $send['method']) {
				add_filter('phpmailer_init', array(get_called_class(), 'mail_smtp'));
			}
			add_action('wp_mail_failed', array(get_called_class(), 'wp_mail_failed'));
		}
		...
		// Clear hooks.
		if (in_array($send['method'], array('smtp','wp_mail'), true)) {
			remove_filter('wp_mail_content_type', array(get_called_class(), 'mail_content_type'));
			if ('smtp' === $send['method']) {
				remove_filter('phpmailer_init', array(get_called_class(), 'mail_smtp'));
			}
			remove_action('wp_mail_failed', array(get_called_class(), 'wp_mail_failed'));
		}
	...
	protected static function send_now_ses($to, $subject, $message, $headers, $attachments) {
		...
		try {
			...
			$m->addTo($to);
			$m->setFrom(static::format_recipient($send['ses']['email'], $send['ses']['name']));
			...
			if (count($headers)) {
				foreach ($headers as $h) {
					...
					if (preg_match('/^bcc:/i', $h)) {
						$m->addBCC(trim(preg_replace('/^bcc:\s*/i', '', $h)));
					}
					elseif (preg_match('/^cc:/i', $h)) {
						$m->addCC(trim(preg_replace('/^cc:\s*/i', '', $h)));
					}
					else {
						$m->addCustomHeader($h);
					}
	...
	public static function mail_smtp($phpmailer) {
		...
		if ('smtp' === $send['method']) {
			...
			$phpmailer->FromName = $send['smtp']['name'];
