...
class template {
	...
	public static function get_slugs() {
		if (is_null(static::$slugs)) {
			global $wpdb;
			static::$slugs = array();
			$dbResult = $wpdb->get_results("SELECT DISTINCT `post_name` FROM `{$wpdb->prefix}posts` WHERE `post_type`='wh-template' AND `post_status`='publish' ORDER BY `post_name` ASC", ARRAY_A);
	...
	public static function get($slug=null, $refresh=false) {
		...
		if ($refresh || !isset(self::$_instances[$class][$slug])) {
			self::$_instances[$class][$slug] = new static($slug);
			if (!self::$_instances[$class][$slug]->is_template()) {
				unset(self::$_instances[$class][$slug]);
				return new static();
			}
		}

		return self::$_instances[$class][$slug];
	}
	...
	public function __construct($slug=null) {
		...
		// And get the content.
		$dbResult = $wpdb->get_results("SELECT `post_content` AS `raw`, `post_name` AS `slug`, `post_title` AS `title` FROM `{$wpdb->prefix}posts` WHERE `post_type`='wh-template' AND `post_status`='publish' AND `post_name`='" . esc_sql($slug) . "'");
		if (!is_array($dbResult) || !count($dbResult)) {
			return false;
		}
	...
	public function is_template() {
		return is_array($this->template) && strlen($this->template['raw']);
	}
	...
	protected function debug($action='', $state=null) {
		...
		// Starting.
		if (is_null($this->log_start) || true === $state) {
			$this->log = array(
	...
	public function make($data=null, $options=null, $raw=null) {
		...
		if ($options['utm'] && !count(array_filter($options['utm_data'], 'strlen'))) {
			$options['utm'] = false;
		}
		...
		if ($options['utm'] || $options['link_target']) {
			...
			if ($options['utm']) {
				...
				$utm_query = count($utm_query) ? implode('&', $utm_query) : false;
			}
			...
			if ($head->length) {
				...
				if ($head->item(0)->hasChildNodes()) {
					...
					for ($x = $length - 1; $x >= 0; $x--) {
						...
						// Skip non-tags.
						if (!isset($child->tagName) || !strlen($child->tagName)) {
							continue;
						}
