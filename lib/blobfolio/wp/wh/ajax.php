...
class ajax {
	...
	public static function register_actions() {
		foreach (static::ACTIONS as $action=>$handler) {
			add_action("wp_ajax_$action", array(get_called_class(), $handler));
		}
	}
	...
	protected static function send($out=null) {
		...
		if (count($out['errors']) && 200 === $out['status']) {
			$out['status'] = 400;
		}
	...
	public static function preview() {
		...
		if (strlen($data['data']) && !common\data::is_json($data['data'])) {
		...
		// Save the data?
		if (!count($out['errors']) && $data['post_id'] > 0) {
			if ($out['data']['json']) {
		...
		if (!count($out['errors']) && $data['email'] && $data['emailTo']) {
			if ($data['post_id'] > 0) {
	...
	public static function pro() {
		...
		if (!current_user_can('manage_options')) {
			$out['errors']['other'] = __('You do not have access to manage licensing.', 'well-handled');
			$out['status'] = 403;
		}
		...
		if (!count($out['errors'])) {
			...
			// Maybe a real one.
				...
				if ($license->is_license()) {
					...
					$out['errors']['license'] = __('The license is not valid.', 'well-handled');
				}
			}
		}
	...
	public static function prune() {
		...
		if (!count($out['errors'])) {
			...
			if ('meta' === $_POST['mode']) {
				$wpdb->query("UPDATE `{$wpdb->prefix}wh_messages` SET `message`='' WHERE DATEDIFF(NOW(), `date_created`) >= {$_POST['age']}");
			}
			// For everything, we need to delete links and messages.
			elseif ('full' === $_POST['mode']) {
				$wpdb->query("DELETE FROM `{$wpdb->prefix}wh_messages` WHERE DATEDIFF(NOW(), `date_created`) >= {$_POST['age']}");
				$wpdb->query("DELETE FROM `{$wpdb->prefix}wh_message_links` WHERE NOT(`message_id` IN (SELECT `id` FROM `{$wpdb->prefix}wh_messages` ORDER BY `id` ASC))");
			}
			// For errors, kill errors.
			else {
				$wpdb->query("DELETE FROM `{$wpdb->prefix}wh_message_errors` WHERE DATEDIFF(NOW(), `date_created`) >= {$_POST['age']}");
			}
	...
	public static function queue() {
		...
		if (!count($out['errors'])) {
			if (options::has('queue')) {
				$out['data']['queue']['total'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_message_queue`");
				$out['data']['queue']['due'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_message_queue` WHERE `date_scheduled` <= NOW()");
			}
	...
	public static function errors() {
		...
		if (!count($out['errors'])) {
			...
			$out['data']['total'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_message_errors` WHERE $conds");
	...
	public static function activity() {
		...
		if (!count($out['errors'])) {
			...
			$out['data']['total'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_messages` AS m WHERE $conds");
			...
			if ($out['data']['total'] > 0) {
				...
				$dbResult = $wpdb->get_results("
					...
					FROM
						...
						LEFT JOIN `{$wpdb->prefix}posts` AS p ON m.template=p.post_name
					WHERE $conds
	...
	public static function stats() {
		...
		$out['data']['date_min'] = $wpdb->get_var("SELECT MIN(DATE(`date_created`)) FROM `{$wpdb->prefix}wh_messages`");
		if (is_null($out['data']['date_min'])) {
		...
		$out['data']['date_max'] = $wpdb->get_var("SELECT MAX(DATE(`date_created`)) FROM `{$wpdb->prefix}wh_messages`");
		...
		foreach ($dbResult as $Row) {
			// Nice method names.
			if ('wp_mail' === $Row['method']) {
				$Row['method'] = 'Default';
			}
			elseif ('ses' === $Row['method']) {
				$Row['method'] = 'Amazon SES';
			}
			elseif ('mandrill' === $Row['method']) {
				$Row['method'] = 'Mandrill';
			}
			else {
				$Row['method'] = 'SMTP';
		...
		// Count emails.
		$out['data']['stats']['sent']['recipients'] = (int) $wpdb->get_var("SELECT COUNT(DISTINCT `email`) FROM `{$wpdb->prefix}wh_messages` WHERE $conds");
