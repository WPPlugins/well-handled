...
// This file is a quick rewrite of HTMLawed. All we're really
// interested in is moving the functions into a namespaced class
// to keep them from polluting the global namespace.
//
// Changes:
// Rename main function to ::filter
// Replace hl_ with static::hl_
// Replace ([^\[])('hl_[^']+') with $1array(get_called_class(), $2)
...
class htmlawed {
	 ...
	 * @return string
	 */
	public static function filter($t, $C = 1, $S = array()) {
	...
	// config rest
		...
		$C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or static::hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or static::hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
		...
		$GLOBALS['C'] = $C;
		$S = is_array($S) ? $S : static::hl_spec($S);
		if (isset($GLOBALS['S'])) {
			$reS = $GLOBALS['S'];
		}
		$GLOBALS['S'] = $S;
		...
		if ($C['cdata'] or $C['comment']) {
			$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', array(get_called_class(), 'hl_cmtcd'), $t);
		}
		$t = preg_replace_callback('`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', array(get_called_class(), 'hl_ent'), str_replace('&', '&amp;', $t));
	...
	// main
		$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', array(get_called_class(), 'hl_tag'), $t);
		$t = $C['balance'] ? static::hl_bal($t, $C['keep_bad'], $C['parent']) : $t;
		$t = (($C['cdata'] or $C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
		$t = $C['tidy'] ? static::hl_tidy($t, $C['tidy'], $C['parent']) : $t;
		unset($C, $e);
		if (isset($reC)) {
			$GLOBALS['C'] = $reC;
		}
	...
	// final $spec
		...
		for ($i = count(($t = explode(';', $t))); --$i >= 0;) {
			...
			foreach (explode(',', $a) as $v) {
				...
				if (isset($y[$x]['match']) && !static::hl_regex($y[$x]['match'])) {
					unset($y[$x]['match']);
				}
				if (isset($y[$x]['nomatch']) && !static::hl_regex($y[$x]['nomatch'])) {
					unset($y[$x]['nomatch']);
				}
			}
			if (!count($y) && !count($n)) {
	...
	// tag transform
		...
		if ($C['make_tag_strict'] && isset($eD[$e])) {
			$trt = static::hl_tag2($e, $a, $C['make_tag_strict']);
			if (!$e) {
				return (($C['keep_bad'] % 2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
			}
		}
		...
		foreach ($aA as $k => $v) {
			if (((isset($d['*']) ? isset($d[$k]) : !isset($d[$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or (isset($aNO[$k]) && !isset($d['on*'])) or (isset($aNA[$k]) && !isset($d['aria*'])) or (!isset($d['data*']) && preg_match('`data-((?!xml)[^:]+$)`', $k))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
				...
				if ($k == 'style' && !$C['style_pass']) {
					...
					$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', array(get_called_class(), 'hl_prot'), $v);
					...
					if ($k == 'srcset') {
						...
						foreach (explode(',', $v) as $k1 => $v1) {
							...
							$v1 = trim($v1[0]);
							if (isset($v1[0])) {
								$v2 .= static::hl_prot($v1, $k).(empty($k1) ? '' : ' '.$k1).', ';
							}
						}
						$v = trim($v2, ', ');
					}
					if ($k == 'itemtype') {
						...
						foreach (explode(' ', $v) as $v1) {
							if (isset($v1[0])) {
								$v2 .= static::hl_prot($v1, $k).' ';
							}
						}
						$v = trim($v2, ' ');
					} else {
					$v = static::hl_prot($v, $k);
					}
					if ($k == 'href') { // X-spam
				...
				if (isset($rl[$k]) && is_array($rl[$k]) && ($v = static::hl_attrval($k, $v, $rl[$k])) === 0) {
					continue;
				}
				$a[$k] = str_replace('"', '&quot;', $v);
			}
