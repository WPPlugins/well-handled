...
class cron {
	...
	public static function register_actions() {
		foreach (static::ACTIONS as $action=>$handler) {
			// Add the action.
			add_action($action, array(get_called_class(), $handler));
	...
	public static function send_queue() {
		...
		$dbResult = $wpdb->get_results("SELECT `id`, `data` FROM `{$wpdb->prefix}wh_message_queue` WHERE `date_scheduled` <= NOW() ORDER BY `id` ASC LIMIT {$send_queue['qty']}", ARRAY_A);
	...
	public static function retention() {
		...
		// This is easy.
		$wpdb->query("UPDATE `{$wpdb->prefix}wh_messages` SET `message`='' WHERE DATEDIFF(NOW(), `date_created`) >= {$send_data['retention']}");
