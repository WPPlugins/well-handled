...
class link {
	...
	public static function get($link_id=null, $refresh=false) {
		...
		if ($refresh || !isset(self::$_instances[$class][$link_id])) {
			self::$_instances[$class][$link_id] = new static($link_id);
			if (!self::$_instances[$class][$link_id]->is_link()) {
				unset(self::$_instances[$class][$link_id]);
				return new static();
			}
		}

		return self::$_instances[$class][$link_id];
	}
	...
	public function __construct($link_id=null) {
		...
		// And get the content.
		$dbResult = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}wh_message_links` WHERE `mask`='$link_id'");
	...
	public function is_link() {
		return is_array($this->link) && strlen($this->link['mask']) > 0;
	}
	...
	public static function link_mask() {
		...
		while (
			...
			intval($wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}wh_message_links` WHERE `mask`='$mask'"))
		) {
			$mask = common\data::random_string(20);
		}
	...
	public static function bulk($message_id, $urls) {
		...
		if (
			$message_id <= 0 ||
			is_null($wpdb->get_var("SELECT `id` FROM `{$wpdb->prefix}wh_messages` WHERE `id`=$message_id"))
		) {
			return false;
		}
		...
		foreach ($chunks as $v) {
			$wpdb->query("INSERT INTO `{$wpdb->prefix}wh_message_links`(`mask`,`message_id`,`url`) VALUES" . implode(',', $v) . ' ON DUPLICATE KEY UPDATE `mask`=`mask`');
		}
